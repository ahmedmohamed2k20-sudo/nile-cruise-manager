...
class _ReservationScreenState extends State<ReservationScreen> {
              ...
              child: provider.isLoading 
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final displayH = displayHour == 0 ? 12 : displayHour;
                        final timeStr = '$displayH:${minute.toString().padLeft(2, '0')} $period';
                        ...
                        return GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(10), border: Border.all(color: color.withOpacity(0.8))),
                            child: Row(
                              children: [
                                SizedBox(width: 75, child: Text(timeStr, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13))),
                                ...
                                Expanded(
                                  child: reservation != null
                                      ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          ...
                                          Text(
                                            '${DateFormat('h:mm a').format(reservation['startTime'])} - ${DateFormat('h:mm a').format(reservation['endTime'])} | ${reservation['employeeName'] ?? ''}',
                                            style: TextStyle(fontSize: 11, color: Colors.grey.shade700),
                                            maxLines: 1, overflow: TextOverflow.ellipsis,
                                          ),
                                        ])
                                      : Text(color == Colors.blue.withOpacity(0.5) ? '🔧 Prep Time' : 'Available', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                                ),
                                if (reservation != null) const Icon(Icons.chevron_right, size: 18),
                              ],
                            ),
                          ),
                        );
  ...
  void _showDetails(Map<String, dynamic> r) {
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (ctx) => Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text('👨‍💼 Added by: ${r['employeeName'] ?? 'Unknown'}'),
      if (r['employeeEmail'] != null) Text('📧 ${r['employeeEmail']}', style: TextStyle(fontSize: 12, color: Colors.grey)),
      Text('🕐 ${DateFormat('h:mm a').format(r['startTime'])} - ${DateFormat('h:mm a').format(r['endTime'])}'),
      if (r['hasDrinks'] == true) Text('🥤 Drinks: ${r['drinksDetails'] ?? 'Yes'}'),
      if (r['hasFood'] == true) Text('🍽️ Food: ${r['foodDetails'] ?? 'Yes'}'),
      if (r['hasSpecialDecorations'] == true) Text('🎉 Decorations: ${r['decoDetails'] ?? 'Yes'}'),
      if (r['hasBirthdayCake'] == true) Text('🎂 Cake: ${r['cakeDetails'] ?? 'Yes'}'),
      if (r['specialNotes'] != null && r['specialNotes'].toString().isNotEmpty) Text('📝 Notes: ${r['specialNotes']}'),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: () {
          ...
          final end = start.add(Duration(minutes: 30 * _dur));
          final provider = Provider.of<ReservationProvider>(context, listen: false);
          if (provider.hasConflict(start, end)) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('❌ This time conflicts with another reservation!'), backgroundColor: Colors.red));
            return;
          }
          final auth = Provider.of<AuthProvider>(context, listen: false);
          provider.addReservation({
            'customerName': _name.text,
            'employeeName': auth.employeeName ?? 'Staff',
